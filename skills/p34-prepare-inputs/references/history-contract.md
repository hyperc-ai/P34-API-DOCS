...
| `business_led` (the default when `grounding_mode` is omitted) | Where a group carries no flag there is no executed batch to reason from. The service treats the quantities up to the group's total recorded sales as determined by the tape, and leaves larger quantities unlabelled unless the business description establishes that the tape is uncensored demand (a marketplace-observed demand series, an order book). So supply the stock and demand facts — what was held, whether it sold out, whether the tape sees demand beyond what you could serve — in the business description. |
