...
| `T=0 keys do not quote their full order range` | on a market whose answer may only use the quantities your T=0 rows quote (t5market.com), default business-led grounding considers every order size from 1 to a key's `max_qty`, so each key must quote that range. Add a T=0 row for every quantity you can order for each key, from 1 to its `max_qty`, with the key's `unit_price` and `unit_cost` — or at least its smallest and largest order. Without a `max_qty` column, quote at least two quantities per key. The detail names how many keys fall short and gives examples; nothing was charged. |
| `carries a single quantity row` | every historical key carries only the quantity that was ordered, on a fit whose rows are used as sent (`client_grounded`, or a business-led fit pinned to a non-default `grounding_executor`). Send the alternative quantity rows the business could have ordered (`profit` blank) — see [The Menus table](03-data-format.md#the-menus-table) — or submit it `business_led` on the default grounding, which adds them. A plausibility check: `checks: "off"` skips it, and the fit then fails on the cluster (below). |
...
| the fit finished, but its answer uses order quantities your T=0 menu did not quote, on a market whose answer may only use quoted quantities (t5market.com) | `result_outside_quoted_options` — terminal, the answer is never issued | list every quantity you can order for each key as its own T=0 row, from 1 to its `max_qty`, then submit a new fit. `/fit` refuses most such menus up front (see [Common 422 errors](#common-422-errors)) |
