...
| `carries a single quantity row` | every historical key carries only the quantity that was ordered. Send the alternative quantity rows the business could have ordered (`profit` blank) — see [The Menus table](03-data-format.md#the-menus-table). A plausibility check: `checks: "off"` skips it, and the fit then fails on the cluster (below). |
